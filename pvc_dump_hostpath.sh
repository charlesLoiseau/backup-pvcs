#!/usr/bin/env sh
# pvc_backup_all.sh
# Back up ALL PVCs (optionally filtered by namespace list or prefix) to a specific node+path using BusyBox.
...
NS_LIST="${NS_LIST:-}"                            # Optional: space-separated namespaces to include explicitly
NS_PREFIX="${NS_PREFIX:-}"                        # Optional: only namespaces starting with this prefix
...
RUN_ID="$(date -u +%Y%m%dT%H%M%SZ)"   # used in archive filenames
...
# Discover namespaces (supports NS_LIST and NS_PREFIX)
discover_namespaces() {
  if [ -n "$NS_LIST" ]; then
    for ns in $NS_LIST; do [ -n "$ns" ] && echo "$ns"; done
...
launch_all() {
  ...
  [ -n "$ns_list" ] || { log "No namespaces found (NS_LIST='${NS_LIST}', NS_PREFIX='${NS_PREFIX}')."; return; }
...
log "Node=${BACKUP_NODE}, Node path=${BACKUP_BASE}, RunID=${RUN_ID}, NS_PREFIX='${NS_PREFIX}', NS_LIST='${NS_LIST}'"
